...
class ScribeError(Exception):
    """Raised when report writing fails or produces no usable output."""
    pass
...
def scribe_write(vault):
    if not vault.synthesis or not vault.synthesis.strip():
        raise ScribeError("No synthesis available in Vault — Synthesizer step likely failed")

    if vault.mode == "topic":
        prompt = TOPIC_PROMPT.format(synthesis=vault.synthesis)
    elif vault.mode == "single_doc":
        prompt = SINGLE_DOC_PROMPT.format(synthesis=vault.synthesis)
    elif vault.mode == "multi_doc":
        prompt = MULTI_DOC_PROMPT.format(synthesis=vault.synthesis)
    else:
        raise ValueError(f"Unknown mode: {vault.mode}")

    try:
        response = llm.invoke(prompt)
    except Exception as e:
        raise ScribeError(f"Groq LLM call failed during report writing: {e}") from e

    content = getattr(response, "content", None)
    if not content or not content.strip():
        raise ScribeError("Scribe LLM returned an empty report")
